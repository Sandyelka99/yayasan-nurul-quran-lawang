    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-100 bg-white/95 shadow-sm backdrop-blur-xl">
      <div className="mx-auto flex min-h-24 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden flex-1 items-center justify-center gap-1 xl:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="whitespace-nowrap rounded-full px-3 py-2 text-[13px] font-semibold leading-none text-slate-700 transition hover:bg-cyan-50 hover:text-cyan-700"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="hidden shrink-0 xl:block">
          <a
            href="/donasi-wakaf"
            className="inline-flex whitespace-nowrap items-center gap-2 rounded-full bg-gradient-to-r from-cyan-700 to-teal-600 px-5 py-3 text-sm font-bold leading-none text-white shadow-lg shadow-cyan-900/15 transition hover:-translate-y-0.5 hover:shadow-xl"
          >
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-xl border border-slate-200 p-2 text-slate-700 transition hover:bg-slate-50 xl:hidden"
        ...
        <div className="border-t border-slate-100 bg-white px-4 py-4 shadow-lg xl:hidden">
          <nav className="flex flex-col gap-2">
            ...
            <a
              href="/admin"
              onClick={() => setIsOpen(false)}
              className="rounded-xl px-4 py-3 text-sm font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-slate-700"
            >
              Admin
            </a>
          </nav>
